...
Instead, the context grows with every recursive call so that \scheme{(fac 6 1)} reduces as
\begin{schemedisplay}
(fac 6 1)
...
\end{schemedisplay}
whence the result will be validated six times---and \scheme{n} times for \scheme{(fac n 1)}.
...
Herman et al.~\cite{herman2010space} observed the potential accumulation of casts in the blame calculus---similar in principle to the accumulation of guards in our contracted \scheme{fac}---and applied the coercion calculus of Henglein~\cite{henglein1994dynamic} to compact them.
...
%\stxclass{p}{Prim}{\scheme|impersonate-operator| $|$ \scheme|<| $|$ \scheme|>| $|$ %\scheme|=| $|$ \scheme|+| $|$ \scheme|-|}{}
\end{tabular}
...
\newcommand{\sapp}[4]{\Sigma_a(#1,#2,\ell,#3,#4)}
\newcommand{\scha}[6]{\Sigma_d(#1,#2,#3,#4,#5,#6)}
\newcommand{\sdimpz}[6]{\Sigma_{imp_0}(#1,#2,#3,#4,#5,#6)}
\newcommand{\sdimpo}[5]{\Sigma_{imp_1}(#1,#2,#3,#4,#5)}
\newcommand{\sdchaz}[6]{\Sigma_{chap_0}(#1,#2,#3,#4,#5,#6)}
\newcommand{\sdchao}[5]{\Sigma_{chap_1}(#1,#2,#3,#4,#5)}
\newcommand{\serr}[3]{\Sigma_!(#1,#2,#3)}
